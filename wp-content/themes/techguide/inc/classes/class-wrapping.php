 ...
 */
add_filter( 'template_include', array( 'Techguide_Wrapping', 'wrap' ), 99 );
add_filter( 'techguide_wrap_base', 'techguide_wrap_base_cpts' );
add_filter( 'techguide_wrap_base', 'techguide_wrap_base_tmpl_dir' );
 ...
 */
function techguide_template_path() {
	return Techguide_Wrapping::$main_template;
}
 ...
 */
function techguide_template_base() {
	return Techguide_Wrapping::$base;
}

if ( ! class_exists( 'Techguide_Wrapping' ) ) {
	...
	class Techguide_Wrapping {
		...
		/**
		 * Initialize new instance of Techguide_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'techguide_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Techguide_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function techguide_wrap_base_cpts( $templates ) {
 ...
 * @return array            Filtered set of templates.
 */
function techguide_wrap_base_tmpl_dir( $templates = array() ) {

	$result   = array();
	$base_dir = 'base-templates/';

	foreach( $templates as $template ) {
		$result[] = $base_dir . $template;
		$result[] = $template;
	}
