 ...
 */
function techguide_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->techguide_comment_list_args = $args;
	$comment = $_comment;
	...
	<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
		<div class="comment-body__holder">
			<?php techguide_get_template_part( 'template-parts/comment' ); ?>
		</div>
 ...
 */
function techguide_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->techguide_comment_list_args['avatar_size'] ) ) {
		$size = $comment->techguide_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'techguide_comment_author_avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function techguide_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'techguide_get_comment_author_link', sprintf( '<h6 class="fn">%1$s</h6>', get_comment_author_link() ), $args );
}
 ...
 */
function techguide_get_comment_date( $args = array() ) {
	...
	if ( isset( $args[ 'human_time' ] ) && $args[ 'human_time' ] ) {
		...
		$suffix = esc_html__( ' ago', 'techguide' );
	}
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'techguide_get_comment_date', sprintf( '<span class="comment-date"><a href="%3$s" class="comment-date__link"><time datetime="%1$s" class="comment-date__time">%2$s%4$s</time></a></span>', get_comment_time( 'c' ), $date, get_comment_link(), $suffix ), $args );
}
 ...
 */
function techguide_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'techguide_get_comment_text', $comment_text, $args );
}
 ...
 */
function techguide_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'techguide_get_comment_reply_link', $reply, $args );
}
 ...
 */
function techguide_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'techguide' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'techguide_get_comment_link_edit', $link, $comment->comment_ID, $args );
}
